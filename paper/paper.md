...
state fidelity $F = 0.977 \pm 0.020$ (seed 17). \autoref{fig:process_tomography}
shows process tomography of a noisy qutrit Hadamard, reconstructed with process
fidelity $F = 0.993 \pm 0.001$ (seed 40). Both benchmarks ship as runnable
examples with fixed seeds, so anyone can reproduce the numbers.
...
**) on a shared color scale. The reconstruction recovers both populations (diagonal) and coherences (off-diagonal), with purity $0.86$ and residuals $|\rho - \hat{\rho}| \le 0.03$. State fidelity in the squared (Jozsa) convention is $F(\rho,\hat{\rho}) = \left[\mathrm{Tr}\sqrt{\sqrt{\rho}\,\hat{\rho}\,\sqrt{\rho}}\right]^2 = 0.977 \pm 0.020$ (mean $\pm$ s.d., 50 shot noise realizations; realization nearest the mean shown, seed 17).\label{fig:state_tomography}](figures/state_tomography.pdf){
width=85% }
...
`choi_to_kraus`), ideal versus reconstructed: one dominant gate operator and eight small depolarizing operators. The true channel is trace-preserving (numerically, $\sum_\ell K_\ell^\dagger K_\ell = I$ to $10^{-15}$); the reconstructed Kraus set, taken from the Choi matrix, is completely positive but not constrained to be trace-preserving. Process fidelity is $F = 0.993 \pm 0.001$ (squared state fidelity between the normalized Choi states $J/d$; mean $\pm$ s.d., 50 shot noise realizations; realization nearest the mean shown, seed 40).\label{fig:process_tomography}](figures/process_tomography.pdf){
width=90% }
...
(Claude Opus 4.6-4.8 via CLI) and Cursor assisted with code review, code testing and debugging, drafting and editing
documentation, and maintaining the test suite.
