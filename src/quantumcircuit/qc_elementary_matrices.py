...
def x_plus() -> np.ndarray:
    return np.array([[0, 0, 1],
                     [1, 0, 0],
                     [0, 1, 0]], dtype=complex)


def x_minus() -> np.ndarray:
...
def hdm(omega: float = np.exp(1j * 2 * pi / 3)) -> np.ndarray:
...
def sdg(omega: float = np.exp(1j * 2 * pi / 3)) -> np.ndarray:
    return np.array([
        [1, 0, 0],
        [0, 1, 0],
...
def tdg(omega: float = np.exp(1j * 2 * pi / 3)) -> np.ndarray:
    return np.array([
        [1, 0, 0],
