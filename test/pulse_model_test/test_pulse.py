...
from src.pulse_creation import Pulse_Schedule, Shift
import os.path
import unittest
...
class TestPulseList(unittest.TestCase):
    ...
    def test_save_pulses(self) -> None:
        ...
        self.full_path_csv = Pulse.pulse_list.save_pulses(saved_type="csv", file_path=test_path)
        self.assertTrue(os.path.isfile(self.full_path_csv))
        with open(self.full_path_csv, 'r') as file:
        ...
        self.full_path_json = Pulse.pulse_list.save_pulses(saved_type="json", file_path=test_path)
        self.assertTrue(os.path.isfile(self.full_path_json))
        with open(self.full_path_json, 'r') as file:
            self.assertTrue(file.readline().startswith("pulse id, mode, duration, frequency,"
                                                       "x_amp, sx_amp, beta_dephase, beta_leakage,"
                                                       "sigma, pulse_pointer"))
    ...
    def tearDown(self) -> None:
        ...
        :return:
        """
        os.remove(self.full_path_csv)
        os.remove(self.full_path_json)


class TestPulseSchedule(unittest.TestCase):
    """
    Test pulse running on quantum computer - ibm
    """
    ...
    def setUp(self) -> None:
        ...
        self.pulse01 = Pulse01(frequency=5, x_amp=5,
                               sx_amp=5, duration=5)
        self.drive_phase = 0.2
        self.drive_beta = 0.5
        self.channel = 1

    def test_single_pulse_schedule(self) -> None:
        """

        :return:
        """
        single_pulse = Pulse_Schedule.single_pulse_schedule(
            pulse_model=self.pulse01,
            drive_phase=self.drive_phase,
            drive_beta=self.drive_beta,
            channel=self.channel
        )
        self.assertIsInstance(single_pulse, ScheduleBlock)
        self.assertEqual(single_pulse.get_parameters('frequency'), self.pulse01.frequency)
        self.assertEqual(single_pulse.get_parameters('duration'), self.pulse01.duration)
        self.assertEqual(single_pulse.get_parameters('beta'), self.drive_beta)
        # TODO: Test Channel

        gaussian_pulse = Pulse_Schedule.single_pulse_gaussian_schedule(
            pulse_model=self.pulse01,
            channel=self.channel
        )
        self.assertIsInstance(gaussian_pulse, ScheduleBlock)
        self.assertEqual(gaussian_pulse.get_parameters('frequency'), self.pulse01.frequency)
        self.assertEqual(gaussian_pulse.get_parameters('duration'), self.pulse01.duration)
        # TODO: Test Channel
