...
See also [
`examples/noise_and_tomography.ipynb`](https://github.com/spham1611/qutritium/blob/main/examples/noise_and_tomography.ipynb)
...
- Kraus channels (gate and prep errors) only run on the `DensityMatrixSimulator`.
  Hand one to the statevector `StatevectorSimulator` and it'll refuse with a
  `NotImplementedError`.
...
More shots, better reconstruction; with only a few, linear least squares can hand back
a slightly unphysical $\rho$ (a small negative eigenvalue) — pass
`method="projected_lls"` (or its alias `"mle"`) to project it onto the closest
physical state.
